...
def naiveSoftmaxLossAndGradient(
    ...
    ### YOUR CODE HERE (~6-8 Lines) # D=3, V=10
    # print(f"centerWordVec shape: {centerWordVec.shape}") # 1xD
    # print(f"outsideVectors shape: {outsideVectors.shape}") # VxD
    
    a = np.dot(outsideVectors[outsideWordIdx], centerWordVec)
    scores = np.exp(np.matmul(centerWordVec, outsideVectors.T)) # 1xV
    b = np.sum(scores)
    loss = -a + np.log(b)
    
    y_hat = scores / b # 1xV

    y = np.zeros(outsideVectors.shape[0]); y[outsideWordIdx] = 1
    gradCenterVec = np.matmul((y_hat - y), outsideVectors)
    
    # print(f"y_hat shape: {y_hat.shape}") # 1xV
    # print(f"gradCenterVec shape: {gradCenterVec.shape}") # Dx1
    
    y_hat[outsideWordIdx] = y_hat[outsideWordIdx]-1
    gradOutsideVecs = np.outer(y_hat, centerWordVec)
    # print(f"gradOutsideVecs shape: {gradOutsideVecs.shape}") # VxD
...
def negSamplingLossAndGradient(
    ...
    For example if an outside word is sampled twice, you shall have to
    double count the gradient with respect to this word. 
    ...
    loss = -np.log(sigmoid(x)) 
    gradCenterVec = (sigmoid(x)-1)*outsideVectors[outsideWordIdx] 
    gradOutsideVecs = np.zeros_like(outsideVectors)
    gradOutsideVecs[outsideWordIdx] = (sigmoid(x) - 1) * centerWordVec
    
    for s in negSampleWordIndices: 
        y = np.dot(outsideVectors[s], centerWordVec) # 1xV 
        loss -= np.log(sigmoid(-y))
        gradCenterVec += (1-sigmoid(-y))*outsideVectors[s]
        gradOutsideVecs[s] += (1-sigmoid(-y))*centerWordVec
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    loss = 0.0
    gradCenterVecs = np.zeros(centerWordVectors.shape)  # VxD
    gradOutsideVectors = np.zeros(outsideVectors.shape) # VxD
    ...
    for j, outsideWord in enumerate(outsideWords):
        outsideWordIdx =  word2Ind[outsideWord]
        loss_j, gradCenter, gradOutside = word2vecLossAndGradient(centerWordVec,
                                                                         outsideWordIdx, 
                                                                         outsideVectors, 
                                                                         dataset)
        loss += loss_j 
